...
def plot_backscatter_timeseries_by_id(timeseries, point_ids, max_points=2):
    """
    ...
    fig, axes = plt.subplots(len(point_ids), 2, figsize=(15, 5*len(point_ids)))
    if len(point_ids) == 1:
        axes = np.array([axes])  # Ensure axes is 2D
    
    # Colors for polarizations
    vv_color = '#ff7f0e'  # Orange for VV (like in the screenshot)
    vh_color = '#1f77b4'  # Blue for VH
...
def plot_backscatter_timeseries_by_coordinates(timeseries, lon, lat, radius_meters=20, max_points=2):
    """
